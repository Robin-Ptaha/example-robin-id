...
const std::string CheckSize(const std::string& value, std::string_view field_name, int min,
    ...
    throw utils::errors::ValidationError{
        field_name, fmt::format("username length must be between {} and {} characters", min, max)};
}
...
const std::string ValidateEmail(const userver::formats::json::Value& data,
    ...
    if (!data.HasMember(field_name)) {
        throw utils::errors::ValidationError{field_name, "required"};
    }

    const auto& email = data[field_name].As<std::string>();
    if (email.empty()) {
        throw utils::errors::ValidationError{field_name, "cannot be empty"};
    }
    ...
    throw utils::errors::ValidationError{field_name, "invalid"};
}

const std::string ValidateUsername(std::string_view username) {
    if (std::regex_match(username.data(), kUsernamePattern)) {
        return CheckSize(username.data(), "username", kUsernameLengthMin, kUsernameLengthMax);
    }

    throw utils::errors::ValidationError{"username", "invalid"};
}
...
const std::string ValidatePassword(const userver::formats::json::Value& data,
    ...
    throw utils::errors::ValidationError{field_name, "at least one uppercase letter, one lowercase letter, one number and one special character"};
}

...
class AuthCheckerBearer final : public userver::server::handlers::auth::AuthCheckerBase {
   public:
    ...
    AuthCheckerBearer(const jwt::JWTConfig& config, bool is_required)
...
AuthCheckerBearer::AuthCheckerResult AuthCheckerBearer::CheckAuth(
    ...
    if (bearer_sep_pos == std::string::npos ||
        std::string_view{auth_header_value.data(), bearer_sep_pos} != "Token") {
        return AuthCheckerResult{AuthCheckerResult::Status::kTokenNotFound,
                                 {},
    ...
    const auto user_id =
        static_cast<std::string>((*decoded_token).get_payload_claim("user_id").as_string());

    request_context.SetData("user_auth_data",
                            UserAuthData{std::move(user_id), std::move(*decoded_token)});
...
userver::server::handlers::auth::AuthCheckerBasePtr CheckerFactory::operator()(
    ...
    const auto is_required =
        auth_config.HasMember("required") ? auth_config["required"].As<bool>() : false;
